...
import { calculateDebt, currentBangkokMonth, debtSentence, formatThb, monthBounds, ownerLabels } from '../../app/ledger'
...
export function DashboardPage() {
  ...
  return (
    <div className="page stack">
      <header className="page-header dashboard-header"><div><p className="eyebrow">{new Date(`${month}-01`).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' })}</p><h1>Сводка расходов</h1><p>{debtSentence(debt)}</p></div><Link className="button primary desktop-add" to="/add">＋ Добавить расход</Link></header>
      <section className="budget-grid" aria-label="Бюджеты месяца">
        {owners.map((owner) => {
          ...
          return <article className="budget-card" key={owner}><div className="card-heading"><h2>{ownerLabels[owner]}</h2><span>{percent}%</span></div><p className="budget-value">{formatThb(spent)} <small>из {budget ? formatThb(budget.totalLimitSatang) : '—'}</small></p><div className="progress"><span style={{ width: `${percent}%` }} /></div>{budget?.categoryLimits.slice(0, 3).map((limit) => { const value = owned.filter((expense) => expense.categoryId === limit.categoryId).reduce((sum, expense) => sum + expense.amountSatang, 0); return <div className="mini-progress" key={limit.categoryId}><span>{categoryMap.get(limit.categoryId) ?? 'Категория'}</span><strong>{formatThb(value)} / {formatThb(limit.limitSatang)}</strong></div> })}</article>
        })}
      </section>
      <section className="section-card"><div className="section-title"><div><p className="eyebrow">Последние траты</p><h2>Что покупали</h2></div><Link to="/expenses">Все расходы</Link></div>{monthExpenses.length === 0 ? <p className="empty-text">В этом месяце расходов ещё нет.</p> : <ul className="expense-list compact">{monthExpenses.slice(0, 5).map((expense) => <li key={expense.id}><span><strong>{expense.merchant}</strong><small>{categoryMap.get(expense.categoryId) ?? 'Категория'} · {ownerLabels[expense.owner]}</small></span><strong>{formatThb(expense.amountSatang)}</strong></li>)}</ul>}</section>
