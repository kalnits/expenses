...
import { formatCurrency, MoneyAmount, requestExchangeRate, useDisplayCurrency } from '../../lib/displayCurrency'
...
export function ExpensesPage() {
  ...
  const update = useMutation({ mutationFn: async ({ id, draft }: { id: string; draft: ExpenseDraft }) => {
    ...
    const { amountMinor, currency: originalCurrency, ...details } = draft
    const expense = await repository.update(id, { ...details, originalAmountMinor: amountMinor, originalCurrency, normalizedMerchant, notes: draft.notes || null, ilsPerThb: null, usdPerThb: null })
