...
import { currentBangkokMonth, formatThb, monthBounds, ownerLabels, sourceLabels } from '../../app/ledger'
...
export function AddExpensePage() {
  ...
  const save = useMutation({ mutationFn: (value: ExpenseDraft) => repository.create(asInput(value, userId)), onSuccess: async () => { await invalidateLedger(queryClient, householdId); navigate('/expenses') } })
...
export function ExpensesPage() {
  ...
  const update = useMutation({ mutationFn: ({ id, draft }: { id: string; draft: ExpenseDraft }) => repository.update(id, { ...draft, normalizedMerchant: draft.merchant.toLocaleLowerCase('ru-RU').trim(), notes: draft.notes || null }), onSuccess: async () => { await invalidateLedger(queryClient, householdId); setEditing(null); setEditDraft(null) } })
